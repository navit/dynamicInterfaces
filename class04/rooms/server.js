...
var socketServer = io.listen(server); // create a socket connection
socketServer.sockets.on('connection',function(socket) {

	/*
socket.on('change room',function(data) {//ready to recieve it
		if(data.oldRoom) {
			socket.leave(data.oldRoom);//new command build in the idea of rooms
			socket.broadcast.to(data.oldRoom).emit('user left',data.username);//.to.emit- sends to every body but the current client- to means evvery body in that room- so we are telling everybody in the old room that a user has left.
		}
*/
